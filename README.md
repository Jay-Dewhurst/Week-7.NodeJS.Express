...
The project separates concerns by keeping API and server logic and frontend assets in the `public` folder seperate.
Express serves the `public` folder directly, and all data requests go through the API route.
...
2. Open a terminal in the project folder or with Bash/mac equivalent and install dependencies using command "npm i" or ""npm install"
   This installs the packages listed in `package.json`, which are `express` and `uuid`.
3. Start the server using command "npm run start"
   This runs `node server.js` as defined in the `scripts` section of `package.json`.
4. Open a browser and go to: http://localhost:3001, the port is set in `server.js` via the `PORT` constant.
